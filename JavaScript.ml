...
and format f =
  format_term_with_precedence (-1) f

and format_term_with_precedence outer_precedence f ast =
  let inner_precedence = precedence ast in
  let format_rec = format_term_with_precedence inner_precedence in
  let format_ast f = function
    ...
    | Call (callee, arguments) -> fprintf f
        "%a(%a)" format_rec callee (format_list format_rec) arguments
